...
const brightestStars = window.starNames;
const countriesData = window.countriesData;

var loc = getGeoLocation("DEU");
var lat = 52.52, lon = 13.40; // Berlin
...
function prepareBackground() {
  ...
  const gradient = ctxBackground.createRadialGradient(
    canvasBackground.width / 2,  // Zentrum X
    canvasBackground.height / 2, // Zentrum Y
    0,                           // Innerer Radius (Startpunkt)
    canvasBackground.width / 2,  // Äußerer Radius (Endpunkt X)
    canvasBackground.height / 2, // Äußerer Radius (Endpunkt Y)
    Math.max(canvasBackground.width, canvasBackground.height) // Maximale Entfernung
  );
...
function drawStars(stars) {
  ...
  stars.forEach(star => {
    const { alt, az } = raDecToAltAz(star.RAICRS, star.DEICRS, lat, lon);
    if (alt > 0) {
      const r = (90 - alt) / 90;
      ...
      const brightness = Math.max(1, 6 - star.Vmag);
      let starColor = "green";
